...
b=json.load(open(S+'benchmarks.json')); br=json.load(open(S+'brands.json'))
...
json.dump(b,open(S+'blob2.json','w'),separators=(',',':'))
open(S+'blob2.js','w').write("const HEBE="+json.dumps(b,separators=(',',':'))+";")
print("rows",len(b['rows']),"brands",len(b['brands']),"scatter",len(b['scatter']))
print("blob bytes",len(open(S+'blob2.js').read()))
